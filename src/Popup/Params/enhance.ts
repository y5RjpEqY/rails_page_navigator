...
export function useEnhance() {
    const { state: { selectedPath }, dispatch } = useContext(StateContext);
    ...
    useEffect(() => {
        dispatch({ type: actionTypes.SET_URL, payload: Object.entries(paramsValue).reduce((acc, [param, value]) => acc.replace(param, value || param),`${host}${selectedPath}`) })
